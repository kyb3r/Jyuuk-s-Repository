...
class ConnectFour:
    ...
    def grid_maker():
        ...
        for i in range(rows):
            ...
            for value in range(cols):
                line.append('\N{MEDIUM BLACK CIRCLE}')
            grid.append(line)
        return grid
    ...
    def player_one(grid,colour):
        ...
        while player1:
            for i in range(len(grid)):
                if grid[i][int(player1)] == '\N{MEDIUM BLACK CIRCLE}':
    ...
    async def display_grid(ctx, p1, p2):
        pass
        
        

    async def start_game(ctx, p1, p2):
        grid = await display_grid(ctx, p1, p2)
        cycles = int(len(grid)*len(grid[0])/2)
        
        
    @commands.command(pass_context=True)
    async def four_row(self, ctx, *players: discord.Member):
        p1, p2 = players
